...
void DBA::GrantUpload(uint32_t idx){
    ...
    uint64_t currentError = 0, averageTotalError = 0, SVError = 0, SD = 0, cycleNum = 0, currentErrorWithoutMTW = 0;
    int64_t averageTotalError8to11 = 0, SVError8to11 = 0, SD8to11 = 0;
    ...
    else if (simTime() > 3)
    {
        ...
        currentError = abs(grantUpold[idx] - onuold[idx] - Rerror[idx]);  //Error with MTW
        ...
        if(cycleCount > 0)
        {
            ...
            //calculate average of Ai
            currentRerror += onuAi[idx];
            if(idx == 31) {
                totalRerror = ((cycleNum8to11 - 1) * totalRerror + currentRerror) / cycleNum8to11;
                currentRerror = 0;
            }
            //
...
*/
    ...
    LogResults op7("GrantUp_ONU16");
      ...
      LogResults op8("AIerror_ONU16");
          ...
          */
    LogResults op66("NewData_Multi_ONU16");
    if( simTime() > 3  && idx == 16 )
    {
        //op6 << "cycleNum8to11 : " << cycleNum8to11 << " currentError8to11 : "<< currentError8to11 << " averageTotalError8to11_PerCycle : " << averageTotalError8to11  << " Standard_Deviation8to11_PerCycle : " << SD8to11 << " Rerror : " << Rerror << endl;
        op66 << " Ri - 1: " << onuold[idx] << "\t Ri : " << onunow[idx] << "\t Gi : "<< grantUp[idx] << "\t Ai - 1 : " << Rerror[idx] << "\t Gi-1 : " << grantUpold[idx] << endl;
    }
...
void DBA::DecideGrantUp(uint32_t idx) { //cc, linear, LSTM
    ///----------constant credit-----------
    ///*
    LogResults try_o("Show_TotalAvgRerror");
    try_o << "totalRerror / 32 = " << totalRerror / 32 << endl;

    uint64_t AvgSingleAi = totalRerror / 32;
    if ( onuRequestSize[idx] <= 0 ) //onuRequestSize[idx]<=0
        grantUp[idx] = 0;
    //else if( (onuRequestSize[idx] + AvgSingleAi - 3000) <= 0 ) {
    //    LogResults mark_o("minusTooManyCounter");
    //    grantUp[idx] = onuRequestSize[idx];
    //}
    else if( (onuRequestSize[idx] + (0.1 * AvgSingleAi)) >= MTW )
        grantUp[idx] = MTW;
    else
        grantUp[idx] = onuRequestSize[idx] + (0.1 * AvgSingleAi);
    //*/
    ...
    */
    ...
    if (x == 0){
        ...
        inputData1[idx][1] = onuAi[idx];
        inputData2[idx][1] = onuRequestSize[idx];
    }
    else
    {
        inputData2[idx][x-1] = onuRequestSize[idx];
        inputData1[idx][x-1] = onuAi[idx];
    ...
    const auto model2 = fdeep::load_model("");
    const auto model16 = fdeep::load_model("0426_U12D00_Poisson_ONU16_onuAi_78332_4layer40cells_noLookBack.json");

    //const auto model = fdeep::load_model("0426_U12D00_Poisson_ONU16_onuAi_78332_4layer40cells_noLookBack.json"); //TimeStep=2 U24D02  keras_model_TimeStepIs_2_U12D02_2021_0618.json
    ...
    float normalize_num_1 = 48133.0;  // U24D02
    //float normalize_num_2 = 43578.0;
    float normalize_num_3 = normalize_num_1;
    for (int i = 0; i < 10 ; i++)   // TimeStep=2
    {
    ...
    const auto result2 = model2.predict(
        {fdeep::tensor(fdeep::tensor_shape(static_cast<std::size_t>(vec[0].size())), vec[0])//,
         //fdeep::tensor(fdeep::tensor_shape(static_cast<std::size_t>(vec[1].size())), vec[1])
        }
    );//std::cout
    const auto result16 = model16.predict(
        {fdeep::tensor(fdeep::tensor_shape(static_cast<std::size_t>(vec[0].size())), vec[0])//,
         //fdeep::tensor(fdeep::tensor_shape(static_cast<std::size_t>(vec[1].size())), vec[1])
        }
    );//std::cout
    ...
    string r2 = fdeep::show_tensors(result2);
    string r16 = fdeep::show_tensors(result16);
    //string r = fdeep::show_tensors(result);
    string res = "";
    if(idx == 2) {
    ...
    float c = std::stof(res);
    //o6 << "idx : " << idx << ", cycleCount : " << cycleCount << ", cycleFromZero : "<< cycleFromZero << ", show_tensors * normalize : "<<  c * normalize_num  << ", onuRequestSize[idx] : " << onuRequestSize[idx] <<   std::endl;
    if ( onuRequestSize[idx] + c * normalize_num_3 < MTW )
        grantUp[idx] = onuRequestSize[idx] + c * normalize_num_3 ;
    else
        grantUp[idx] = MTW;
    if (grantUp[idx] < 0)
        grantUp[idx] = 0;

    currentPredictAi[idx] = c * normalize_num_3 ;

    o6 << "idx : " << idx << ", cycleCount : " << cycleCount << ", cycleFromZero : "<< cycleFromZero << ", show_tensors * normalize : "<<  c * normalize_num_3  << ", onuRequestSize[idx] : " << onuRequestSize[idx] << ", grandUp[idx] : " << grantUp[idx] <<  std::endl;

    //vec[0].erase(vec[0].begin(),vec[0].end());
